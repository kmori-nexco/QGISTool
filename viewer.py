...
class PhotoViewerPlus:
    ...
    def _ensure_points_fields(self, lyr):
    ...
    def show_image(self, idx: int):
        ...
        # left(Frontエリア) は「ひとつ前」の優先（front→back フォールバック）
        if (row.lat_kp is not None) and (row.lon_kp is not None) and prev_row and _same_street(prev_row, row):
            cand = [
                _pick_from_row(prev_row, "front"),
                _pick_from_row(prev_row, "back"),
            ]
        else:
        ...
        # right(Backエリア) は「ひとつ先」の優先（back→front フォールバック）
        if (row.lat_kp is not None) and (row.lon_kp is not None) and next_row and _same_street(next_row, row):
            cand = [
                _pick_from_row(next_row, "back"),
                _pick_from_row(next_row, "front"),
            ]
        else:
    ...
    def _on_image_dblclick(self, ev, label):
        if not self.images:
            return
        # 表示中の実ファイルを優先
        if label is self.img_label_front:
            pth = getattr(self, "_last_disp_front", None)
        else:
            pth = getattr(self, "_last_disp_back", None)
        if pth:
